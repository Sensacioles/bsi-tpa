...
public class ArvoreBinaria2<Tipo extends Comparable<Tipo>>{
    Node<Tipo> raiz;
    ArvoreBinaria2(Node<Tipo> root){
        this.raiz = null;
    }
    // CHAMADA DA PATH ENTRADA
    public void main(String[] args) throws IOException {
 
        String path = "C:\\Users\\x\\Documents\\bsi-doo-main\\bsi-tpa-master\\arvorebinaria\\Entradas.txt";
        ler(path);
    ...
    public void ler(String path ) throws IOException{
        ...
        Tipo matricula = (Tipo) obj[0];     
        addNode( matricula );   

        for (int i=0;i <=numero_matriculas-1;i++){
            linha = buffRead.readLine();
            obj = linha.split( ";");
            matricula = (Tipo)obj[0];
            addNode(matricula);
        }
        buffRead.close();
